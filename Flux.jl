...
function calculate_optimal_flux_distribution(stoichiometric_matrix::Array{Int32,2}, Flux_Array::Array{Float64,2}, Species_Array::Array{Int32,2}, Objective_Array::Array{Int32,1}; min_flag::Bool = true)
    ...
    (number_of_fluxes,number_of_bounds) = size(Flux_Array)
    for flux_index = 1:number_of_fluxes

    	flux_lower_bound = Flux_Array[flux_index,1]
    	flux_upper_bound = Flux_Array[flux_index,2]
    ...
    for (flux_index,obj_coeff) in enumerate(Objective_Array)
    ...
    for species_index = 1:number_of_species
    	...
    	species_lower_bound = Species_Array[species_index,1]
    	species_upper_bound = Species_Array[species_index,2]
